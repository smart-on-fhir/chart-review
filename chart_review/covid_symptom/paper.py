...
def table3_true_prevalence(self):
    ...
    table3 = ['variant_era,covid_symptom,cnt,prevalence_apparent,prevalence_true,sensitivity,specificity']
        ...
        for line in f.readlines():
            if not line.startswith('variant_era'):
                ...
                if covid_symptom in score.keys():
                    ...
                    prevelance = agree.true_prevalence(float(prct), float(sensitivity), float(specificity))
                table3.append(f'{variant_era},{covid_symptom},{cnt},{prct},{prevelance},{sensitivity},{specificity}')
...
def publish_supplement(self):
    ...
    icd10_range_alon = icd10_range.intersection(set(list(NoteRange.amy_alon.value) +
                                                    list(NoteRange.andy_alon.value)))
